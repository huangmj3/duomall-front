...
export default {
  ...
  methods: {
    ...
    addShoppingCartBtn () {
      ...
      // params.append('uid', '3');
      // params.append('goodsId', data.goods_id);
      // params.append('goodsCover', data.package.img);
      // params.append('goodsUnitPrice', data.package.price);
      // params.append('goodsNum', data.count);
      // params.append('status', "3");
      var param = {'uid':userId,'goodsId': data.goods_id,'goodsNum':data.count};
      axios.post('http://huangmj.top/back/cart/add',param).then(function(resp){
        // console.log(resp);
        if(resp.status == 200) {

    ...
    <div class="add-info-box-container">
      <div class="add-info-box">
        <div class="add-info-detail">
          ...
          <div class="add-info-box-row" v-for="(item,index) in shoppingCart" :key="index">
            <div class="add-info-img">
              <img :src="item.goodsCover" alt="">
            </div>
            <div class="add-info-intro">
              <p>{{item.goodsTitle}}...</p>
              <p class="add-info-intro-detail">颜色：{{item.goodsIntro}}/ 数量：{{item.goodsNum}}</p>
            </div>
          </div>
        </div>
...
export default {
  ...
  created () {
    ...
    // alert(userId);
      axios.post('http://huangmj.top/back/cart/list',{uid:userId}).then(function(resp){
          console.log(resp);
          _this.ShoppingCart = resp.data.list
